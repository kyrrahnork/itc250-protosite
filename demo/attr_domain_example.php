<?php
/**
 * attr_domain_example.php based on demo_edit.php is a single page web application 
 * that allows us to select a customer and edit their data
 * 
 * This example shows two ways to pre-load data from a valid range of choices (an attribute domain)
 *
 * One version of the domain is loaded from the app, the other from a database table of countries
 *
 * @package nmCommon
 * @author Bill Newman <williamnewman@gmail.com>
 * @version 1.13 2015/06/16
...
{//Select Customer
	...
	$sql = "select CustomerID,FirstName,LastName,Email,Title,Country from test_Customers cu left join test_Countries co 
on cu.CountryID = co.CountryID";

	/*
	
	select Country from test_Customers cu inner join test_Countries co 
on cu.CountryID = co.CountryID
...
{# shows details from a single customer, and preloads their first name in a form.
	...
	$cObj = buildCountries();//creates the countries for the select element 

	$myID = (int)$_POST['CustomerID'];  //forcibly convert to integer

	$sql = sprintf("select CountryID,CustomerID,FirstName,LastName,Email,Title from test_Customers WHERE CustomerID=%d",$myID);
	$result = mysqli_query(IDB::conn(),$sql) or die(trigger_error(mysqli_error(IDB::conn()), E_USER_ERROR));
	if(mysqli_num_rows($result) > 0)//at least one record!
	{//show results
	...
	$valuStr = "Mr.,Mrs.,Ms.";
	$lblStr = "Mister,Missus,Ms.";
	createSelect("select","Title",$valuStr,$Title,$lblStr,",");
	...
	$countries = 'Pick A Country~' . implode('~',$cObj->countries);
...
function updateExecute()
{
	...
	$CountryID = strip_tags(iformReq('CountryID',$iConn));
